            ...
            {activeTab === 'about' && (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                  <ContactItem icon={<Mail className="text-pink-500" />} text="shubhranshu2192@gmail.com" />
                  <ContactItem icon={<Phone className="text-pink-500" />} text="+91 9169238044" />
                  <ContactItem icon={<Github className="text-pink-500" />} text="github.com/shubhranshu-pandey" />
                  <ContactItem icon={<Linkedin className="text-pink-500" />} text="linkedin.com/in/shubhranshu-pandey21" />
                </div>
              </div>
            )}
...
const ContactItem = ({ icon, text }) => {
  ...
  const isPhone = text.includes('+91');
  const isGithub = text.includes('github.com');
  const isLinkedin = text.includes('linkedin.com');

  const getLink = () => {
    if (isEmail) return `mailto:${text}`;
    if (isPhone) return `tel:${text}`;
    if (isGithub) return `https://${text}`;
    if (isLinkedin) return `https://${text}`;
    return null;
  };
